...
export function JournalVending({onBack,focused}:{onBack():void;focused:boolean}){
  ...
  const pages=Math.max(1,Math.ceil(entries.length/8)),items=entries.slice(page*8,page*8+8);
  ...
  return <div className={styles.machine} data-state={sharing?"form":selected?"detail":"shelf"} aria-label="Chocolate blog selection screen">
    {!focused?<div className={styles.attract}><h2>Blogs</h2><ShopOtter/><p>A little treat for your brain.</p></div>:<>
      <header className={styles.header}><h2>Blogs</h2><button onClick={onBack} aria-label="Leave vending machine"><Icon kind="close"/></button></header>
      ...
      :selected?<article className={styles.detail}><div className={styles.detailProduct}><Chocolate index={Math.max(0,entries.findIndex(entry=>entry.id===selected.id))} label={String(entries.findIndex(entry=>entry.id===selected.id)+1).padStart(2,"0")}/><strong>{selected.title}</strong></div><div className={styles.detailCopy}><span>BLOG</span><h3>{selected.title}</h3><p>{new URL(selected.url).hostname.replace(/^www\./,"")}</p><p>Unwrap the full story on the author’s website.</p></div><a href={selected.url} target="_blank" rel="noopener noreferrer" onClick={()=>worldSound.play("paper",{gain:.15})}>Read blog ↗</a></article>
      :<div className={styles.shelf} aria-busy={loading}>
        {items.map((entry,i)=><button className={styles.card} key={entry.id} onClick={()=>choose(entry)} aria-label={`Choose chocolate: ${entry.title}`}><Chocolate index={page*8+i} label={String(page*8+i+1).padStart(2,"0")}/><strong>{entry.title}</strong></button>)}
        {!items.length&&Array.from({length:8},(_,i)=><div className={`${styles.card} ${styles.unstocked}`} key={i} aria-hidden="true"><Chocolate index={i} label="DD"/><strong>{loading?"Loading…":"Coming soon"}</strong></div>)}
        {error&&<p className={styles.error} role="alert">{error}</p>}
      </div>}
      <footer className={styles.footer}><button className={styles.back} onClick={back}><strong>Go Back</strong><i><Icon kind="back"/></i></button>{!selected&&!sharing&&<div className={styles.prompt}><span>{entries.length?"Choose a chocolate to get started …":"Fresh blogs coming soon …"}</span><button className={styles.add} onClick={()=>{setSharing(true);setError("");}} aria-label="Add a blog"><Icon kind="add"/></button></div>}{pages>1&&!selected&&!sharing&&<nav aria-label="Blog pages"><button disabled={page===0} onClick={()=>setPage(p=>p-1)} aria-label="Previous chocolates">‹</button><span>{page+1}/{pages}</span><button disabled={page===pages-1} onClick={()=>setPage(p=>p+1)} aria-label="More chocolates">›</button></nav>}</footer>
    </>}
  </div>;
}
