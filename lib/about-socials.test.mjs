...
test('About restores all four social icons and keeps X on the correct account',()=>{
  ...
  assert.match(source,/aria-label="Contact" title="Contact"/);
  assert.doesNotMatch(source,/<span>/,'only icons are visible; names remain accessible');
  ...
  assert.match(css,/flex-wrap: wrap/);assert.match(css,/height: 28px/);assert.match(css,/width: 16px; height: 16px/);assert.match(css,/padding: 0;/);assert.match(css,/:focus-visible/);
});
