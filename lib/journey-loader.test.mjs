...
test('loading shows Exit for desktop and Begin for world, without automatically entering',()=>{
  const source=read('../components/JourneyLoader.tsx');
  assert.match(source,/canBegin\s*\?\s*<button/);
  assert.match(source,/const actionLabel=journey.destination==="desktop"\?"Exit":"Begin"/);
  assert.match(source,/aria-label=\{actionLabel\}.*onClick=\{begin\}/);
  assert.match(source,/Array.from\(actionLabel\)/);
  assert.match(source,/Ready to \$\{actionLabel.toLowerCase\(\)\}/);
