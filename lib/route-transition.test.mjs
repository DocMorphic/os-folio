...
test('direct visits and return trips share one loader without duplicate journeys',async()=>{
  ...
  let path;navigateImmersive('/#about',p=>{path=p;});assert.equal(path,undefined);journeyGameReady();await journeyGamePainted();assert.equal(path,'/#about');assert.equal(isReturningToDesktop(),true);
