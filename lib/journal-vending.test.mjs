...
test('chocolates use real blogs, accessible cards and an explicit read link',()=>{
  ...
  assert.match(ui,/const PAGE_SIZE=4/);
  assert.match(ui,/entries.slice\(page\*PAGE_SIZE,\(page\+1\)\*PAGE_SIZE\)/);
  assert.match(ui,/setSelected\(entry\)/);
});
test('catalogue uses compact two-column cards and preserves full titles in details',()=>{
  const css=readFileSync(new URL('../components/JournalVending.module.css',import.meta.url),'utf8');
  const ui=readFileSync(new URL('../components/JournalVending.tsx',import.meta.url),'utf8');
  assert.doesNotMatch(css,/repeat\(4,|grid-auto-rows:minmax/);
  assert.match(css,/grid-auto-rows:49cqw/);assert.match(css,/-webkit-line-clamp:3/);
  assert.match(css,/overflow-wrap:normal;word-break:normal/);
  assert.match(ui,/<h3>\{selected.title\}<\/h3>/);
  assert.match(ui,/title=\{entry.title\}/);assert.match(ui,/sourceName\(entry.url\)/);
  assert.doesNotMatch(ui,/Choose a chocolate to get started/);
});
